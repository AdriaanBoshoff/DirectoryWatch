...
/// Authors:         Angus Johnson, angusj-AT-myrealbox-DOT-com, Adriaan Boshoff.
/// Description:     Implements watching for file changes in a directory.
/// Version:         1.5
/// Date:            2018-08-28
/// Target:          Win32, Delphi 10.2.3
...
type
  TNotifyFilters = set of (nfFilename, nfDirname, nfAttrib, nfSize, nfLastWrite,
    nfSecurity);
  ...
  published
    ...
    property NotifyFilters: TNotifyFilters read fNotifyFilters
      write SetNotifyFilters;
  ...
  public
    constructor Create(OwnerHdl: THandle; const InitialDir: string;
      WatchSubDirs: boolean; NotifyFilters: dword);
    destructor Destroy; override;
    procedure Terminate;
...
begin
  if aNotifyFilters = fNotifyFilters then
    exit;
...
begin
  if aWatchSubDirs = fWatchSubDirs then
    exit;
...
begin
  ...
  begin
    ...
    fWatchThread := TWatchThread.Create(fWindowHandle, fDirectory,
      fWatchSubDirs, nf);
  end
  else
  begin
...
constructor TWatchThread.Create(OwnerHdl: THandle; const InitialDir: string;
  WatchSubDirs: boolean; NotifyFilters: dword);
begin
...
begin
  ...
  while not fFinished do
    sleep(10); // avoids a reported resource leak
...
begin
  ...
  begin
    ...
    begin
      ...
      until Terminated or (WaitForSingleObject(fChangeNotify, TEN_MSECS) <>
        WAIT_OBJECT_0);
      if Terminated then
        break;
